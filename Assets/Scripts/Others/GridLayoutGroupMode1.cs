...
{
    [AddComponentMenu("Layout/Grid Layout Group Mode 1", 152)]
    /// <summary>
    ...
    /// </remarks>
    public class GridLayoutGroupMode1 : LayoutGroup
    {
        /// <summary>
        ...
        protected GridLayoutGroupMode1()
        {}
        ...
        private void SetCellsAlongAxis(int axis)
        {
            ...
            for (int i = 0; i < rectChildrenCount; i++)
            {
                var cellSizeX = rectChildren[i].sizeDelta[0];
                var cellSizeY = rectChildren[i].sizeDelta[1];

                SetChildAxis(rectChildren[i], 0, offset.x, cellSizeX);
                SetChildAxis(rectChildren[i], 1, offset.y, cellSizeY);

                offset.x += cellSizeX + spacing[0];
                //offset.y += cellSizeY + spacing[1];
            }
        }
        ...
        private void SetChildAxis(RectTransform rect, int axis, float pos, float size)
        {
            ...
            anchoredPosition[axis] = (axis == 0) ? (pos + size * rect.pivot[axis] * 1f) : (-pos - size * (1f - rect.pivot[axis]) * 1f);
